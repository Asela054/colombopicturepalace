								...
								<form action="#" method="post"
									...
									<div class="form-group mt-2">
										<button type="button" name="BtnAdd" id="BtnAdd"
											class="btn btn-warning btn-sm font-weight-bold fa-pull-right"><i
								...
								<div class="form-group mt-2">
									<button type="button" name="Btnsubmit" id="Btnsubmit"
										class="btn btn-primary btn-sm  fa-pull-right"><i
											class="fas fa-exchange-alt"></i>&nbsp;Transfer Stock</button>
								</div>
							</div>
						</div>
    ...
    $(document).ready(function() {
        ...
        $('#fromlocation').on('change', function () {
        	...
        	$.ajax({
        		...
        		url: '<?php echo base_url("Stocktransfer/Getproductlist"); ?>',
        		success: function (result) {
        			...
        			obj.forEach(function (product) {
        				options += '<option value="' + product.idtbl_print_material_info + '">' + product.materialname + ' - ' + product.materialinfocode + '</option>';
        			});
        			$('#productlist').html(options);

        			$('#productlist').on('change', function () {
        				var selectedProduct = obj.find(product => product.idtbl_print_material_info == $(this).val());
        				$('#hiddenproductid').val($(this).val());
        				$('#hiddenproductcode').val(selectedProduct.materialinfocode);
        			});
        		},
...
$('#productlist').on('change', function () {
    ...
    $.ajax({
        ...
        url: '<?php echo base_url("Stocktransfer/Getbatchlist"); ?>',
        success: function (result) {
            var obj = JSON.parse(result);
            var options = '';
            ...
            $('#batchlist').on('change', function () {
                var selectedBatch = obj.filter(product => $(this).val().indexOf(product.idtbl_product_stock));
...
$(document).on("click", "#Btnsubmit", function () {
	...
	Swal.fire({
		title: "Are you sure?",
		text: "You want to process this stock transfer?",
		icon: "warning",
		...
		if (result.isConfirmed) {
			...
			Swal.fire({
				title: "Processing...",
				text: "Please wait while the stock transfer is being processed.",
			...
			$.ajax({
				...
				url: "<?php echo base_url() ?>Stocktransfer/Stocktransferprocess",
				success: function (result) {
					var objfirst = JSON.parse(result);

					Swal.fire({
						title: "Transaction Successful!",
						text: "The stock transfer has been completed successfully.",
						icon: "success",
						confirmButtonColor: "#3085d6"
					}).then(() => {
